...
- **Accessibility** — WCAG 2.1 AA is the explicit target in ACCESSIBILITY.md, with a full page-level checklist, component ARIA pattern map, and known gaps documented (dialog focus trap, `aria-pressed`, live region). Mobile nav drawer shipped 30 Jul 2026 (FEAT-003). TICKET-100 is the pre-launch pass gate. Story Board and map modal gaps are on me, not Dave's backlog.
