...
| 11 | **Once reviews pass, ahead of step 4** | **Decommission the third-party draft deployments** (HC-081). Six of Dave's draft page deploys and Lorna's Website→Keela→Finance map are live on public Netlify URLs — verified 6 Aug 2026 as HTTP 200 with **no `robots` meta and no `X-Robots-Tag`**, i.e. fully indexable near-complete copies of contentment.org content with no canonical back to us. **Timing decided by Somesh:** the takedown request goes out only once review feedback has passed, because these sites *are* the review surface. The risk of leaving them indexable until then is accepted knowingly. **Zero-cost middle ground if the review window stretches:** `X-Robots-Tag: noindex` via a `_headers` file, or Netlify password protection — removes the SEO exposure while leaving the sites fully usable for reviewers. **These sit on Dave's and Lorna's own Netlify accounts — we cannot action any of it ourselves.** |
