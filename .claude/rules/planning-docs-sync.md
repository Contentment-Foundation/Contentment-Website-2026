...
## Status vocabulary (tickets + HC) — added 5 Aug 2026

A status must answer **"is anyone on our side moving on this?"** Before 5 Aug, twelve tickets sat at `In Progress` whether the remaining work was ours or someone else's, so the Sheet could not tell "Somesh is building this" from "this has sat on one person for a week."

| Situation | Status | Also required |
|---|---|---|
| We are actively working it | `In Progress` | — |
| Ours, finished | `Done` | — |
| Ours, prepared, waiting on a date | `Scheduled` (tickets) / `Ready` (HC) | — |
| **Fully stopped on another person** | `Blocked` | **Name them in `Waiting on`** |
| **Part landed, rest owed by another person** | `Partial` | **Name them in `Waiting on`** |
| Deliberately not scheduled, needs a decision first | `Open` | Name the decision owner |

- **Always name a person, never a team alone** — "Lorna — per-tier Keela products", not "waiting on Finance".
- **Tickets have a `Waiting on` column.** The **Handoff Checklist does not and must not** — `LaunchPlanSheet.gs` hardcodes its legend panel at column **J** with Notes at **H**, so a 9th column silently collides with the legend. HC puts `WAITING ON:` / `ON US:` as the **first line of Notes** instead.
...
python3 scripts/refresh-timelines.py --notes          # → both timeline HTMLs (commit these)
```
...
| File | Update when |
|------|-------------|
| `DEV-TIMELINE.html` | **Generated — do not hand-edit status.** Run `refresh-timelines.py --notes`; it rewrites the 25 ID-keyed status spans, their notes and the `As of` stamp from the JSON |
| `dev-timelinev2.html` | Same script does both files in one pass — they can no longer drift apart |
...
2. Run `build-sheet-script.py`, `refresh-timelines.py --notes` (and canvas refresh when JSON changed). `refresh-timelines.py --check` exits 1 if the timelines are stale — cheap pre-commit gate.
...
- Don't hand-edit timeline **status spans, note cells or the `As of` stamp** — use `refresh-timelines.py`. Everything else in those files (design, phase grouping, Gantt, chapter prose) is still hand-authored and safe to edit. A genuine divergence from JSON goes in that script's `PINS`, which prints its reason on every run so it cannot rot.
